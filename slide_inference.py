...
from slide_inference.patch_to_slide import stitch_patches_to_multilevel_tiff_alternative

def create_black_image(image_path, width=512, height=512):
    """
...
def run_pipeline(slide_path, model_path, temp_path, output_path, type, k):
    ...
    slide_to_patches(slide_path, patch_size=512, save_dir=patch_dir, num_workers=4)
    ...
    batch_inference(patch_dir, model_path, vis_dir, k=k, max_workers=16)
    ...
    stitch_patches_to_multilevel_tiff_alternative(
        slide_path,
        vis_dir,
        output_path,
        patch_size=512
    ...
    print(f"  Total time:           {patch_time + norm_time + extract_time + stitch_time:.2f}s")
    print("=" * 50)
