...
def load_model(model_path):
    ...
    # 加载 ResNet-152 并保留其最后的层
    resnet152 = models.resnet152(pretrained=True)  # 使用预训练的 ResNet-152
    resnet152_layers = list(resnet152.children())[:-2]  # 移除最后的全连接层和池化层

    # 定义骨干网络
    backbone = nn.Sequential(*resnet152_layers)
    backbone.out_channels = 2048
    model.backbone = backbone  # 将 ResNet-152 作为骨干网络

    # 再添加一个 classifier 层来适应分割任务
    num_classes = 2
    ...
    class CustomDeepLabV3(nn.Module):
        ...
        def forward(self, x):
            ...
            out = F.interpolate(out, size=(1024, 1024), mode='bilinear', align_corners=False)
            return {"out": out}

    # # 使用自定义模型
    custom_model = CustomDeepLabV3(model)
...
def segment_tissue(model, image_batch, device):
...
def nec_expert_model(dataset_dir, model_path, nec_tissue=["PUMA_0000_0000.png", "PUMA_0007_0000.png", "PUMA_0100_0000.png", "PUMA_0101_0000.png"]):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = load_model(model_path)
    model.to(device)

    for nec in tqdm(nec_tissue):
        image_batch = preprocess_image(join(dataset_dir, "imagesTr", nec))
        predict_path = join(dataset_dir, "predicts", nec.replace("_0000.png", ".png"))
        predict = Image.open(predict_path).convert('L')
        predict_array = np.array(predict)
        output = segment_tissue(model, image_batch, device)
        output = output.squeeze().cpu().numpy()
        output = output.astype(np.uint8)
        Image.fromarray(output).save(join(dataset_dir, "nec", nec))
        mask = (output == 1)
        predict_array[mask] = 5
        Image.fromarray(predict_array).save(predict_path)
...
#     nnUNet_raw="/home/disk3/lwh/image_process/puma/PUMA-challenge-baseline-track2-master/nnunetv2/nnunetv2_hist/nnUNet_raw"
#     nnUNet_results="/home/disk3/lwh/image_process/puma/PUMA-challenge-baseline-track2-master/nnunetv2/nnunetv2_hist/nnUNet_results"
#     model_path = "/home/disk3/lwh/image_process/puma/model_weight/deeplabv3+opt9.pth"
...
#         )
#     predictor.initialize_from_trained_model_folder(
#         join(nnUNet_results, 'Dataset526_Mark/nnUNetTrainer__nnUNetPlans__2d'),
#         use_folds=(4, ),
#         checkpoint_name='checkpoint_best.pth',
#     )
#     predictor.predict_from_files(join(nnUNet_raw, 'Dataset001_PUMA/imagesTs'),
#                                  join(nnUNet_raw, 'Dataset001_PUMA/predictsTs'),
#                                  save_probabilities=False, overwrite=False,
#                                  num_processes_preprocessing=2, num_processes_segmentation_export=2,
#                                  folder_with_segs_from_prev_stage=None, num_parts=1, part_id=0)
#     nec_expert_model(join(nnUNet_raw, "Dataset001_PUMA"), model_path, nec_tissue=["PUMA_0000_0000.png", "PUMA_0007_0000.png", "PUMA_0100_0000.png", "PUMA_0101_0000.png"])
